	...
	void SeekTarget()
	{
		...
		if (_target != null)
		{
			if (_target.tag == "Coin")
			{
                ...
                if (_target.GetComponent<Coin>().isGround && transform.position.y >= _target.transform.position.y && _canGo)
				{
					reachTarget = true;
			...
			else if (_target.tag == "Chest")
			{
				...
				if (_target.GetComponent<Coin>().isGround && transform.position.y >= (_target.transform.position.y - 0.5f))
				{
                    bool _canGo = false;
                    _hitWall = Physics2D.Raycast(new Vector3(transform.position.x - 0.35f * transform.localScale.x,
                      transform.position.y + 0.5f, transform.position.z)
	...
	private void OnCollisionEnter2D(Collision2D collision)
	{
		...
		if (collision.gameObject.tag == "Goblin")
		{
			...
			if (hasWeapon)
			{
				...
				collision.gameObject.GetComponent<EnemySoldier>().GetDamage();
			...
			{
				isDied = true;
				collision.gameObject.GetComponent<EnemySoldier>().SwitchState(EnemySoldier.PlayerState.Attack);
