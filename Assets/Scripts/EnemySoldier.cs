...
public class EnemySoldier : MonoBehaviour
{
	...
	IEnumerator EndTheLevel()
	{
		...
		CurrentLevel._instance._hero.isDied = true;
		CurrentLevel._instance._hero.SwitchState(Player.PlayerState.Die);
	...
	private void OnCollisionEnter2D(Collision2D collision)
	{
		...
		if (collision.gameObject.tag == "Magma")
		{
				...
				&& GameManager.instance._totalGoblinKilled == GameManager.instance._totalGoblin)
			{
				...
				if (_hero != null)
					_hero.GetComponent<Player>().SwitchState(Player.PlayerState.Win);
			}	
